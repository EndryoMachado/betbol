...
//Método de Conexão//
    ...
    public Conexao() {
        try {
            ...
            connection = DriverManager.getConnection("jdbc:mysql://localhost" + "/bdprojeto?useTimezone=true&serverTimezone=UTC", "root", "root");
            System.out.println("conectou!");
        } catch (ClassNotFoundException ex) {
            System.out.println("Nao foi possıvel encontrar a classe " + "do Driver do MySQL " + ex);
        } catch (SQLException ex) {
            System.out.println("Nao foi possıvel conectar ao servidor" + ex);
        }
        try {
            statement = connection.createStatement();
        } catch (SQLException ex) {
